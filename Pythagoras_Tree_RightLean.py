...
def drawNode(t, size, level):
  ...
  if (level < 1):
    ...
    leftSize = size * math.sqrt(3) / 2  # re-computed using Pythagoras formula for balanced tree
    t.forward(size)
    t.right(90)
    t.forward(size)
    t.left(150)  # if left-side angle is 45 degrees, this angle is 180-45=135
    t.forward(leftSize)
    t.right(90)
    ...
    t.right(60)  # if right-side angle is 45 degrees, this angle is = 45
    t.back(size)
